...
public class BrightStarRenderer extends GLSurfaceView implements Renderer {
	...
	int nrOfStarObjects;
	int nrOfObjects;
	int nrOfStringsConstellations;
	int nrOfStringsBorders;
	int nrOfStrings;
	int width;
	int height;
	/*
...
/*adding variables*/
	...
	public BrightStarRenderer(Context context) {
		...
		nrOfStringsConstellations = 168;
		nrOfStringsBorders = 89;
		nrOfStrings = nrOfStringsConstellations + nrOfStringsBorders;
...
/*star draw*/		
		...
		if(mConstellationVisible){
			for (int i=0;i<nrOfStringsConstellations;i++)
			{
				gl.glDisable(GL10.GL_TEXTURE_2D);
			...
			for (int i=nrOfStringsConstellations;i<nrOfStrings;i++)
			{
				gl.glDisable(GL10.GL_TEXTURE_2D);
...
/*for test*/
	...
	public void initBorders() {
		...
		//int num_string = RD.length;
		for (int i=0;i<nrOfStringsBorders;i++) {
			int len_string = RD[i].length;
			float line_coords[] = new float[len_string*3];
			int count = 0;
			for (int j=0;j<len_string;j++){
			...
			ByteBuffer vbb = ByteBuffer.allocateDirect(line_coords.length * 4);
			vbb.order(ByteOrder.nativeOrder());
			vertexLineBuffer[nrOfStringsConstellations + i] = vbb.asFloatBuffer();
			vertexLineBuffer[nrOfStringsConstellations + i].put(line_coords);
			vertexLineBuffer[nrOfStringsConstellations + i].position(0);
		}
    }
